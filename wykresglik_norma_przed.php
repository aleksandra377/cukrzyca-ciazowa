...
$sql = "SELECT `pomiar`, `data_pom` FROM $current_gl_table WHERE `meal`='Przed posilkiem' AND `pomiar`>=70 AND `pomiar`<=90 ORDER BY `data_pom` ASC";
...
<script>
  ...
  const data = {
    ...
    datasets: [{
      ...
      backgroundColor: 'rgb(167,215,148)',
      data: MyData,
    }]
  };
